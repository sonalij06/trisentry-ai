...
const MATCH_SCHEMA = {
  ...
  properties: {
    ...
    immediateSuggestion: { type: Type.STRING },
    whatToWatchFor: { type: Type.STRING },
  },
  required: ["matchedRuleIds", "summary", "immediateSuggestion", "whatToWatchFor"],
};
...
//
// immediateSuggestion/whatToWatchFor are deliberately constrained to general
// actions (seek care, rest, monitor, positioning) - the prompt explicitly
// forbids naming or dosing any medication, same discipline as the
// medication module's labelDosage boundary.
export async function matchSymptomRules(freeText, rules) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({
      model: MODEL,
      contents: `A patient describes their symptoms below. Compare against this fixed catalog of warning signs and return:
- "matchedRuleIds": which catalog ids apply (by id only - never invent new ids)
- "summary": a plain-language, empathetic 1-2 sentence explanation connecting their specific described symptoms to why this is or isn't concerning
- "immediateSuggestion": one practical next step, specific to what THIS patient actually described - name the body part, symptom, or likely cause category they mentioned rather than generic wording. For anything critical, always lead with calling emergency services. For anything non-critical, tailor the self-care action to the actual symptom (e.g. a sprained ankle gets rest/ice/elevation advice, an upset stomach gets bland-diet advice, a skin rash gets keep-it-clean-and-dry advice) instead of defaulting to generic "rest and monitor" phrasing unless that's genuinely the most fitting action. General safe actions only - never name or dose a specific medication.
- "whatToWatchFor": one sentence naming a symptom that would mean they should seek care sooner than currently indicated, if there is one worth naming
...
"""${freeText}"""`,
    ...
    return {
      ...
      immediateSuggestion: parsed.immediateSuggestion ?? "",
      whatToWatchFor: parsed.whatToWatchFor ?? "",
      usedAi: true,
    };
  } catch (err) {
    ...
    return {
      matched,
      summary: "AI call failed - showing keyword-based match instead.",
      immediateSuggestion: "",
      whatToWatchFor: "",
      usedAi: false,
    };
